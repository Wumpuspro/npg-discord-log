...
const pageEnd = `
    <br>
                ...
                Want to add cool events? Submit a pull request on <a href="https://github.com/Wumpuspro/npg-discord-logs">Github</a>!
            </div>
        </div>
    </div>
</body>
...
(async () => {
  ...
  await asyncForEach(
    ...
    async (handler) => {
      ...
      const handlerTitle = parsedComments[0].tags.find(
        (tag) => tag.title === "handler"
      ).description;
      const handlerDescription = `Events related to the ${
        parsedComments[0].tags.find((tag) => tag.title === "related")
          .description
      } event.`;
      pageContent += `
        <h3>${handlerTitle}</h3>
            ...
            blockComment.tags.find((f) => f.title === "event").description
                    ...
                    <div id="event-${pointer}" class="panel-collapse collapse">
                            ...
                            <td style="padding:0 15px 0 15px;">
                                <a target="_blank" href="${
                                  param.type.name.startsWith("DJST:")
                                    ...
                                    ? `https://discord.js.org/#/docs/main/master/class/${param.type.name.slice(
                                        4
                                      )}`
                      ...
                      <div class="panel-body">🆘 Example:<br><pre><code class="javascript">${
                        blockComment.examples[0]
                          ? blockComment.examples[0].value.slice(1)
                          : "No example provided."
                      }</code></pre></div>
                    </div>
                    </div>
